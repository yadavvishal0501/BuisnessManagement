...
if(empty($email) || empty($password) || empty($firstname)|| empty($lastname) || empty($mobile)||empty($address)){
  ...
  if(empty($email)) {
    ...
    if(empty($firstname)) {
    echo "<span color='red'>firstname  field is empty.</font><br>";
    }
 ...
 while($res=mysqli_fetch_array($resu)){
  ...
  <tr>
    ...
    <td>
      ...
      <a  href="deletep.php?id=<?php echo $res['id']; ?>"   class="btn btn-success"  onclick='return confirm("Are You Sure ?")' >Delete</a>
    </td>
  </tr>
  <?php
}
